...
void flash_transfer_trace(const char* message,
                          const flash_transfer_buffer* transfer) {
#if FLASH_TRACE_TRANSFER
  if (flash_trace) {
    size_t c;
    printf(" %s: length=%ld in=%ld out=%ld padding=%ld\n", message,
           transfer->length, transfer->in, transfer->out, transfer->padding);
...
static inline uint8_t flash_Get8(const uint8_t* data) {
  return *data;
}

static inline uint16_t flash_Get16(const uint8_t* data) {
  return (((uint16_t)data[1]) << 8) | data[0];
}

static void flash_TransferBuffer_Clear(flash_transfer_buffer* transfer) {
  transfer->length = 0;
  transfer->padding = 0;
  transfer->in = 0;
  transfer->out = 0;
...
static void flash_TransferBuffer_SetLength(flash_transfer_buffer* transfer,
                                           size_t length) {
...
static flash_error flash_TransferBuffer_Fill(flash_transfer_buffer* transfer,
                                             const uint8_t data,
                                             size_t length) {
...
static flash_error flash_TransferBuffer_Set8(flash_transfer_buffer* transfer,
                                             const uint8_t data) {
...
flash_error flash_transfer_buffer_skip(flash_transfer_buffer* transfer,
                                       const size_t size) {
...
static flash_error flash_TransferBuffer_Get8(flash_transfer_buffer* transfer,
                                             uint8_t* data) {
...
static flash_error flash_TransferBuffer_Get16(flash_transfer_buffer* transfer,
                                              uint16_t* data) {
  if ((transfer->length - (transfer->out - transfer->padding)) <
      sizeof(uint16_t))
    return FLASH_BUFFER_UNDERFLOW;
...
static flash_error flash_TransferBuffer_CopyOut(flash_transfer_buffer* transfer,
                                                uint8_t* data,
                                                const size_t length) {
...
static flash_error flash_TransferBuffer_CopyIn(flash_transfer_buffer* transfer,
                                               const uint8_t* data,
                                               const size_t length) {
#ifdef FLARE_ZYNQMP
...
static void flash_TransferBuffer_SetAddr(flash_transfer_buffer* transfer,
                                         uint32_t address) {
#if FLASH_4BYTE_ADDRESSING
...
static flash_error flash_readRegister(uint8_t reg, uint16_t* value) {
  flash_error fe;
...
static flash_error flash_WaitForWrite(uint32_t wait) {
  uint32_t checks = 100;
  while (true) {
    uint16_t status;
    flash_error fe;
...
static flash_error flash_SetWEL(void) {
  uint16_t status;
  flash_error fe;
...
static flash_error flash_ClearWEL(void) {
  uint16_t status;
  flash_error fe;
...
static flash_error flash_readCFI(void) {
  ...
  printf("   Flash CFI: ID: %02x %02x %02x Flash size: 0x%llx\n",
         flash_Get8(cfi_data + 0), flash_Get8(cfi_data + 1),
         flash_Get8(cfi_data + 2), (long long int)flash_size);

  return fe;
}

static flash_error flash_FindRegion(uint32_t address, uint32_t* base,
                                    size_t* length) {
  flash_error fe;
  size_t region;
...
flash_error flash_open(const char** label) {
  ...
  *label = NULL;
  ...
  fe = flash_read_id(&manufacture_code, &mem_iface_type, &density);
  if (fe != FLASH_NO_ERROR) {
...
flash_error flash_read(uint32_t address, void* buffer, size_t length) {
  ...
  while (length) {
    ...
    flash_transfer_buffer_skip(flash_buf,
...
flash_error flash_writeSector(uint32_t address, const void* buffer,
                              size_t length) {
  flash_error fe = FLASH_NO_ERROR;
...
flash_error flash_read_id(uint32_t* manufactureCode, uint32_t* memIfaceType,
                          uint32_t* density) {
  uint8_t value = 0;
  flash_error fe;
