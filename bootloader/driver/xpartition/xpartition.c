...
void xpart_load_parts() {
    ...
    for (partition = 1; partition < partition_count; partition++) {
        ...
        status = read_boot_bin(
            img.parts[partition].part_hdr_off * sizeof(uint32_t),
            &part_hdr,
        ...
        if (((part_hdr.attr & XLNX_PART_HDR_PART_OWNER_MASK) != XLNX_PART_HDR_PART_OWNER_FSBL) ||
            ((part_hdr.attr & XLNX_PART_HDR_DEST_CPU_MASK) != XLNX_PART_HDR_DEST_CPU_A53_0) ||
        ...
        for (int sec = 0; sec < part_hdr.sec_count; sec++) {
            ...
            status = read_boot_bin(
                sec_hdr.data_off * sizeof(uint32_t),
            ...
            status = read_boot_bin(
                sec_hdr.next_part_off * sizeof(uint32_t),
                &sec_hdr,
                sizeof(struct part_header)
            );
            if (status != 0) {
