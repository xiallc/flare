...
uint32_t factory_load_image_get32(uint8_t* table, uint32_t index,
                                  uint32_t offset) {
  return *((uint32_t*)IMAGE_HEADER_RECORD(table, index, offset));
}

uint8_t factory_load_image_get8(uint8_t* table, uint32_t index,
                                uint32_t offset) {
...
void platform_factory_booter(uint8_t* header, size_t header_size) {
  ...
  memcpy(name, (uint8_t*)FLARE_IMAGE_STAGE_ADDR + UBOOT_IMAGE_NAME_OFF,
         UBOOT_NAME_LEN);
